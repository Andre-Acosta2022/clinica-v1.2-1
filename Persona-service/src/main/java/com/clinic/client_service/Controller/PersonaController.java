...
public class PersonaController {
    ...
    public ResponseEntity<Page<GetPersonaDTO>> getAllPersonsByFullName(
            @PageableDefault(size = 10, sort = "surname", direction = Sort.Direction.ASC) org.springframework.data.domain.Pageable pageable,
            @RequestParam(required = false) String fullName) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<GetPersonaDTO> updatePerson(@PathVariable Long id, @RequestBody UpdatePersonaDTO updatePersonDTO) {
